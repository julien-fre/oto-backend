...
### Consommation et Coûts, lus sur la TENTATIVE (oto-backend#943, 13/09/2026)

**Une source unique : `runner_job_attempts`**, partagée avec les journaux (oto#196) et le
budget (oto#197), au contrat `oto-runner/docs/tentatives-journaux-budget.md` §1 et §3.
Une ligne par tentative ; « run », « agent », « passage », « organisation » n'en sont
que des regroupements. ⚠️ **Livraison par bascule dure** (§4 du contrat) : aucun mode
compatible, un worker qui ne porte pas `attempt_id` ne conclut plus rien.

**Le claim ouvre la tentative, dans UNE transaction** (`db.claim_next_job`) : les
épaves balayées ferment leur tentative `lost` ; `attempts + 1` ; la tentative encore
ouverte du travail repris est close `lost`, usage NULL ; la capacité décide du travail
servi — charge, porteur, clé — sur la connexion de la réservation ; le jeton délégué est
ÉMIS en dernier, sur cette même connexion, rattaché à la tentative par son libellé
(`runner job <travail> tentative <n>`) — une réservation qui refuse n'en émet aucun, une
réservation qui échoue l'emporte avec sa transaction ; les jetons des prises précédentes
sont révoqués (le zombie perd le droit d'écrire au nom du déclarant) ; la tentative est
insérée avec son payeur (`key_source` : `org` si la clé de
l'org est servie, `platform` si un worker de plateforme tourne sur la sienne, NULL
sinon — jamais un défaut) ; `attempt_id` est rendu. Une décision qui lève annule toute la
réservation. Le **run** est celui où la tentative a réellement travaillé : repris s'il
est ouvert, aucun s'il est clos (le détachement ci-dessus).

**Le fencing** : `bind_run`, `extend` et `complete` exigent `attempt_id`
(`attempt_id_required` sinon, avant toute écriture) et n'agissent que par la tentative
COURANTE ouverte. Une tentative close ou remplacée est refusée `409 attempt_superseded`,
avec `details.usage_enregistre`, et le travail ne bouge pas. Un `complete` tardif d'une
tentative `lost` remplit, en une écriture, ses postes restés NULL, leur couverture et le
modèle — rien si aucun poste n'est rempli. Rejouer le même `complete` rend le résultat
enregistré (`replayed: true`), sans rien recompter ni libérer.

**Un refus du SERVEUR au claim** (porteur absent ou invalide, clé exigée absente, worker
sans dépôt) s'écrit sur la connexion de la réservation — une seconde connexion ne verrait
pas la réservation non validée, ne toucherait aucune ligne et perdrait le refus en
silence — et ferme la tentative `failed` : `stopped` = le code du refus
(`requester_missing`, `requester_invalid`, `org_model_key_missing`,
`worker_names_no_key_provider`), postes à **0 attestés par le serveur**
(`usage_couverture.source = "serveur"`). Aucun travail servi, donc zéro certain. Un
refus du WORKER après service n'en est pas un : seul son `complete` déclare.

- **Aucune clé étrangère, aucun identifiant de personne** : `runner_jobs.run_id` part en
  cascade avec l'élagage des runs, et une trace de Coûts qui partirait avec eux ne
  fonderait rien. La table est **durable**, jamais élaguée ; `worker_sub` y est
  l'identité d'une machine. Au retour arrière d'une bascule, ces faits sont conservés.
- **NULL n'est pas 0** : chaque poste est stocké tel que reçu. Sans `usage_couverture`,
  la tentative n'est **pas attestée** : ses nombres restent, rangés à part, jamais
  additionnés au total.
- **Deux mesures, jamais confondues** : la **Consommation** (les jetons, poste par poste)
  et les **Coûts** (un montant en nano-dollars entiers, figé à l'écriture avec le nom de
  son barème, `runner_prix`). Le montant se calcule sur les POSTES par famille, jamais
  sur l'unité de budget `U`.
- ⚠️ **Prix PROVISOIRE ≠ prix ABSENT.** Un barème non vérifié (`mistral-large-2512`)
  tarife, et le montant porte `price_unverified` — à la ligne, à la part, au total. Un
  prix absent (cache lu Mistral non sourcé), un poste requis inconnu, un modèle sans
  tarif ou une famille non nommée rendent `null` et leur raison (`unpriced_reason`).
- **`U` (budget)** : Anthropic = entrée non cachée + cache écrit + sortie ; `mistral` =
  entrée non cachée + sortie (le cache écrit y est sans objet) ; `null` dès qu'un poste
  qui la compose manque.
- **Zéro attesté** : tous les postes reçus à 0, et attestés, valent `U = 0` et un coût
  nul quelle que soit la famille, NULL comprise, sans `unpriced` — c'est ce qui laisse
  un refus au claim compter pour zéro plutôt que « non tarifé ».
- **Lecture réservée aux administrateurs de l'org** (`ORG_ADMIN`, super_admin membre par
  escalade) ; un membre reçoit le refus nommé de la garde. ⚠️ Un opérateur qui CONSULTE
  une org tierce par `X-Oto-Org` est refusé `view_as_read_only` : les `op` de lecture de
  cette capacité (`run`, `agent`, `fleet`, `org`) ne figurent pas dans `_READ_OPS`.
- Sur la clé d'une org (`key_source=org`), le montant est une **estimation au tarif
  public** : nous ne voyons pas son tarif négocié.

Preuves : `tests/test_runner_attempts.py`, `tests/test_runner_attempts_db.py`,
`tests/test_runner_cost.py`, `tests/test_runner_cost_db.py`.
