...
class _FauxHook:
    ...
    def __init__(self, url, auth_token=None, timeout=None):
...
def test_lot_continue_sur_422_sarrete_sur_401(env, monkeypatch):
    ...
    def init(self, url, auth_token=None, timeout=None):
        orig(self, url, auth_token, timeout)
...
# --- revue du 25/09 : compteur par URL, budget d'horloge -----------------------

def test_la_liste_repart_de_zero_pour_un_webhook_recolle(env):
    """Le compteur ne vaut que pour l'URL qu'il a comptée : après un webhook
    recollé, la liste ne doit plus annoncer l'ancien compteur (ni un `remaining`
    presque nul alors que `clay_push_rows` accepterait)."""
    env["entries"]["Leads"]["meta"] = {"kind": "table", "submissions": 49_990,
                                       "submissions_url": C._url_mark(HOOK + "-ancien")}
    t = env["fn"]("clay_list_tables")()["tables"][0]
    assert (t["rows_sent"], t["remaining"]) == (0, C.WEBHOOK_LIMIT)


def test_la_liste_garde_le_compteur_du_webhook_courant(env):
    env["entries"]["Leads"]["meta"] = {"kind": "table", "submissions": 120,
                                       "submissions_url": C._url_mark(HOOK)}
    t = env["fn"]("clay_list_tables")()["tables"][0]
    assert (t["rows_sent"], t["remaining"]) == (120, C.WEBHOOK_LIMIT - 120)


def test_un_webhook_illisible_rend_un_compteur_inconnu(env, monkeypatch):
    def _panne(prov, account=None):
        raise RuntimeError("coffre indisponible")
    monkeypatch.setattr(C.access, "resolve_credential_fields", _panne)
    t = env["fn"]("clay_list_tables")()["tables"][0]
    assert (t["rows_sent"], t["remaining"]) == (None, None)


def test_le_pire_post_d_un_lot_tient_sous_le_plafond_rest(env):
    """Le budget n'est vérifié qu'avant chaque POST : un POST lancé juste avant
    l'échéance dure encore tout son délai. Budget + pire POST ≤ plafond REST."""
    assert C.BATCH_BUDGET_S + sum(C.WEBHOOK_TIMEOUT) < C.REST_CALL_LIMIT_S
