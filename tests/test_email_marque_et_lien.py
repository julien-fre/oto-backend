...
- le TEXTE disait « ouvrir dans oto » — un faux pour quelqu'un qui ne connaît que
  Acme, et qu'aucune URL correcte ne rattrape.
...
def test_avec_lien_le_bouton_et_lurl_en_clair():
    html = email._bouton("https://app.acme.test/org/196/projects/12", "ouvrir")
    assert "https://app.acme.test/org/196/projects/12" in html
...
def test_la_marque_du_destinataire_remplace_oto_partout(monkeypatch):
    ...
    email.send_resource_shared_email(
        "qui@acme.test", type_label="projet", name="P", permission="read",
        app_url="https://app.acme.test", sharer="A", brand="Acme")
    assert "Acme" in envoye["subject"] and "oto" not in envoye["subject"]
    assert "ouvrir dans Acme" in envoye["html"]
