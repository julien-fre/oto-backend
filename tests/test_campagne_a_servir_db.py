...
def _dans_l_ordre(candidates):
    """L'ordre le plus simple, celui de la lecture. Ces bancs éprouvent la REQUÊTE ; ce
    qu'`ordonner` décide s'éprouve dans `test_ordre_de_service.py` (la loi) et
    `test_lignes_reservables_db.py` (les files)."""
...
# ── L'ordre vient d'`ordonner`, l'éligibilité se revérifie sous verrou ────────
# Depuis le 14/09/2026, `campagne_a_servir` ne décide plus de l'ordre : elle tente
# les candidates dans celui qu'`ordonner` rend, et n'en sert aucune qu'il écarte.
# La loi du tirage (file, plancher contre la famine) ne vit plus ici.

def test_l_ordre_rendu_par_ordonner_est_suivi(org_neuve):
    from oto_mcp import db
    ...
    assert db.campagne_a_servir(org, lambda _c: [b["id"], a["id"]])["id"] == b["id"]
    assert db.campagne_a_servir(org, lambda _c: [a["id"], b["id"]])["id"] == a["id"]


def test_une_campagne_qu_ordonner_ecarte_n_est_pas_servie(org_neuve):
    from oto_mcp import db
    f = _flotte(org_neuve["org"], org_neuve["sub"])
    db.armer(f["id"], org_neuve["org"])

    assert db.campagne_a_servir(org_neuve["org"], lambda _c: []) is None


def test_ordonner_recoit_la_declaration_complete_des_candidates(org_neuve):
    """Le compte des files a besoin du tableau visé, du déclarant et du filtre."""
    from oto_mcp import db
    f = _flotte(org_neuve["org"], org_neuve["sub"], row_filter={"passe": "2"})
    db.armer(f["id"], org_neuve["org"])
    vues: list = []

    db.campagne_a_servir(org_neuve["org"], lambda c: vues.extend(c) or _dans_l_ordre(c))

    (vue,) = [c for c in vues if c["id"] == f["id"]]
    assert (vue["namespace"], vue["sub"], vue["row_filter"]) == (
        "t", org_neuve["sub"], {"passe": "2"})


def test_une_flotte_hors_eligibilite_ne_sort_jamais_meme_tentee_la_premiere(org_neuve):
    """La contre-épreuve : une flotte au plafond reste écartée même quand
    `ordonner` la place en tête — l'éligibilité se REVÉRIFIE au moment du verrou,
    pas seulement à la lecture initiale des candidates."""
    from oto_mcp import db
    org, sub = org_neuve["org"], org_neuve["sub"]
    ...
    servie = db.campagne_a_servir(org, lambda _c: [epuisee["id"], servable["id"]])

    assert servie is not None and servie["id"] == servable["id"], (
        "la flotte au plafond ne doit jamais sortir, même tentée la première")
...
def test_sans_org_on_sert_la_campagne_d_une_AUTRE_organisation(org_neuve):
    ...
    assert db.campagne_a_servir(None, _dans_l_ordre) is not None, (
