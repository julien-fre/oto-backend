...
La délégation se fait en deux temps, dans la transaction de la réservation : le
porteur est VÉRIFIÉ (`_verifier_porteur`), et le jeton n'est ÉMIS (`_emettre_jeton`)
qu'une fois toutes les décisions prises — un travail refusé n'en reçoit aucun.
"""
...
def _base(monkeypatch):
    ...
    La doublure accepte la connexion de la réservation (`conn=`) : le refus s'écrit
    sur elle, jamais sur une seconde connexion."""
    refus = []
    monkeypatch.setattr(RJ.db, "refuser_pour_identite",
                        lambda jid, par, raison, **_: refus.append((jid, par, raison)))
...
def test_un_travail_sans_porteur_est_refuse_et_la_raison_est_ecrite(_base):
    job = RJ._verifier_porteur({"id": 7, "org_id": 42, "sub": None}, "le-worker")
    ...
    assert job["_refus"] == RJ.REFUS_SANS_PORTEUR, \
        "le code du refus part avec le travail : la tentative close le recopie"
...
def test_il_n_emporte_ni_jeton_ni_cle(_base):
    ...
    job = RJ._avec_cle(RJ._verifier_porteur({"id": 8, "org_id": 42}, "w"), "anthropic", "w",
                       worker=True)
...
def test_le_refus_dit_quoi_faire_pas_seulement_ce_qui_manque(_base):
    ...
    job = RJ._verifier_porteur({"id": 9, "org_id": 42}, "w")
...
def test_un_travail_qui_nomme_son_porteur_passe_comme_avant(monkeypatch):
    ...
    monkeypatch.setattr(RJ.db, "purger_delegations_expirees", lambda sub, **_: None)
    monkeypatch.setattr(RJ.db, "create_api_token", lambda *a, **k: "oto_delegue")
    monkeypatch.setattr(RJ.db, "refuser_pour_identite",
                        lambda *a, **k: pytest.fail("aucun refus attendu"))
    verifie = RJ._verifier_porteur({"id": 10, "org_id": 42, "sub": "alexis",
                                    "attempts": 1}, "w")
    assert "delegation_refusee" not in verifie
    assert RJ._emettre_jeton(verifie, 600)["delegated_token"] == "oto_delegue"
