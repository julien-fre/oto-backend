...
class _FauxYousignClient:
    ...
    def __init__(self, **kw):
        ...
        self.signataires = {}
        self.supprimees = []
        self.appels = []          # (méthode, kwargs) — l'ordre des appels compte
        self.ancres = None        # None = une ancre par signataire (PDF bien formé)
        self.leve_sur = {}        # méthode → exception à lever
        self.telecharge = None    # None = les octets du document
        self._n = 0

    def _peut_lever(self, methode):
        if methode in self.leve_sur:
            raise self.leve_sur[methode]
    ...
    def create_signature_request(self, name, *, delivery_mode="email", **body):
        self.appels.append(("create_signature_request", body))
        self._peut_lever("create_signature_request")
        sr_id = self._id("sr")
    ...
    def add_document(self, signature_request_id, file_bytes, filename, *,
        ...
        self.appels.append(("add_document", fields))
        self._peut_lever("add_document")
        doc_id = self._id("doc")
        ancres = (len(self.demandes[signature_request_id]["signers"])
                  if self.ancres is None else self.ancres)
        self.documents[doc_id] = {"id": doc_id, "filename": filename, "bytes": file_bytes,
                                  "total_anchors": ancres}
    ...
    def add_signer(self, signature_request_id, info, *,
        ...
        self.appels.append(("add_signer", fields))
        self._peut_lever("add_signer")
        sg_id = self._id("sg")
    ...
    def download_document(self, signature_request_id, document_id):
        if self.telecharge is not None:
            return self.telecharge
        return self.documents[document_id]["bytes"]

    def delete_signature_request(self, signature_request_id, *, permanent_delete=False):
        self._peut_lever("delete_signature_request")
        self.supprimees.append(signature_request_id)
        self.demandes[signature_request_id]["status"] = "deleted"
        return None


class _Credential:
    """Ce que rend `access.resolve_credential` : les champs déballés."""
...
def mcp(monkeypatch):
    monkeypatch.setattr(access, "resolve_credential",
                        lambda provider, want="auto", **kw: _Credential(key="k-test"))
    import oto.tools.yousign as pkg
    faux = _FauxYousignClient()
    construits = []

    def _construire(**kw):
        construits.append(kw)
        return faux
