...
3. **la durée de vie** — les révisions d'une ligne supprimée restent, celles d'un
   tableau supprimé partent ;
...
def _revisions(ns_id: int, row_id: str = "r1") -> list[dict]:
    return _sql("SELECT rev, diff, acteur, run_id, source, geste_id "
...
def test_suppression_de_ligne_garde_ses_revisions(live):
    from oto_mcp import db
    ns_id = _table({"a": 1})
    _ecrire(ns_id, {"a": 2})
    db.datastore_delete_row(ns_id, "r1")
    assert _sql("SELECT 1 FROM datastore_rows WHERE ns_id = %s", ns_id) == []
    assert [r["rev"] for r in _revisions(ns_id)] == [0, 1]
...
def test_rejouer_la_pose_ne_recree_rien(live):
    ...
    assert {journal_revisions.DECLENCHEUR_INSERT,
            journal_revisions.DECLENCHEUR_UPDATE} <= noms
...
def _pose(dsn: str) -> dict:
    """Ce que porte la base : la table, la fonction, les déclencheurs."""
    import psycopg

    from oto_mcp.db import journal_revisions as j
    with psycopg.connect(dsn) as c:
        table = c.execute("SELECT to_regclass(%s) IS NOT NULL", (j.TABLE,)).fetchone()[0]
        fonction = c.execute("SELECT count(*) FROM pg_proc WHERE proname = %s",
                             (j.NOM_FONCTION,)).fetchone()[0] > 0
        declencheurs = {r[0] for r in c.execute(
            "SELECT tgname FROM pg_trigger WHERE tgrelid = 'datastore_rows'::regclass "
            "AND tgname = ANY(%s)", ([j.DECLENCHEUR_INSERT, j.DECLENCHEUR_UPDATE],))}
    return {"table": table, "fonction": fonction, "declencheurs": len(declencheurs)}


_TOUT = {"table": True, "fonction": True, "declencheurs": 2}
_RIEN = {"table": False, "fonction": False, "declencheurs": 0}
...
def test_la_revision_pose_la_table_se_defait_et_le_boot_repose_tout(live, pg_module_dsn):
    ...
    with psycopg.connect(pg_module_dsn, autocommit=True) as c:
        ...
        c.execute(f"DROP FUNCTION {j.NOM_FONCTION}()")
    ...
    assert _revisions(ns_id) == [{"rev": 2, "diff": {"a": {"avant": 2, "apres": 3}},
                                  ...
                                  "geste_id": None}], "le journal n'écrit plus"
