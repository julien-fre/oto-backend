...
et `docs`, et qu'une ancienne copie restée dans `nodes` ne se sert plus.
"""
import json
...
@pytest.mark.parametrize("cas", ["autre_personne", "inconnu", "projet_archive", "copie"])
def test_un_refus_est_le_meme_404_qu_un_inconnu(base, cas):
    ctx, ident = {
        ...
        "copie": (ResolvedCtx(sub=MOI), base["copie"]),
    }[cas]
