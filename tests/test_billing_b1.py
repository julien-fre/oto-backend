...
def test_init_drops_legacy_table_before_schema():
    ...
    # ⚠️ On inspecte la fonction qui APPLIQUE le schéma, pas `init_db`. Elle a changé
    # de nom deux fois pour la même raison — le corps descend d'un cran chaque fois
    # qu'on ajoute une couche autour : `init_db` → `_init_db_once` (retry
    # anti-deadlock) → `apply_boot_schema` (ADR 0065 lot 0 : la séquence devient
    # rejouable hors du démarrage, cf. `replay_boot_schema_dry`). Chercher les deux
    # repères dans la mauvaise fonction casse le CI sur un invariant pourtant intact.
    import inspect
    from oto_mcp.db import _init

    src = inspect.getsource(_init.apply_boot_schema)
