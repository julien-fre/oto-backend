...
`providers.org_secret_meta` est la fonction de service réutilisée par l'adaptateur
...
def test_shareable_provider_no_base_url_ok():
    meta, code = providers.org_secret_meta("serper", None)
    assert code is None
...
def test_slack_now_org_shareable():
    ...
    meta, code = providers.org_secret_meta("slack", None)
    assert code is None
    assert meta is None


def test_unknown_provider_refused():
    meta, code = providers.org_secret_meta("does-not-exist", None)
...
def test_data_driven_remote_with_base_url_ok_and_stripped():
    ...
    meta, code = providers.org_secret_meta("some-bridge", "https://bridge.example.com/")
    assert code is None
...
def test_unknown_provider_without_base_url_not_remote():
    ...
    meta, code = providers.org_secret_meta("some-bridge", None)
    assert code == "provider_not_shareable"
    assert meta is None


def test_normal_connector_rejects_base_url():
    meta, code = providers.org_secret_meta("attio", "https://nope.example.com")
    assert code == "base_url_not_allowed"
    assert meta is None


def test_all_org_shareable_providers_accepted_without_base_url_unless_remote():
    remote = {c.name for c in providers.REMOTE_CONNECTORS}
    for provider in providers.ORG_SHAREABLE_PROVIDERS:
        meta, code = providers.org_secret_meta(provider, None)
