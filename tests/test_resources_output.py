...
def test_resource_type_n_a_plus_de_defaut_implicite():
    """Il valait `datastore_namespace`, relique du pilote ADR 0030. Un appelant qui
    visait un projet et oubliait le champ interrogeait silencieusement une AUTRE
    famille — et sur `transfer`/`share`, agissait sur une autre ressource. Un défaut
    sur un discriminant ne rend pas l'appel plus court, il le rend faux."""
    assert R.ResourceInput.model_fields["resource_type"].is_required()
    with pytest.raises(ValidationError):
        R.ResourceInput(op="list")


@pytest.mark.parametrize("famille", ["datastore_namespace", "project", "doctrine"])
@pytest.mark.parametrize("valeur", ["abc", "", "7a", "1 OR 1", "../7"])
def test_un_identifiant_non_numerique_est_refuse_a_l_entree(famille, valeur):
    """UN seul comportement pour les trois familles, et c'est un 400.

    Avant #659 il y en avait trois, dont deux faux : `int(rid)` levait un
    `ValueError` NON RATTRAPÉ (l'adaptateur REST ne connaît qu'`AuthzDenied`) pour un
    tableau et pour un projet — **500**, une faute du serveur annoncée pour une
    faute du client — tandis qu'un guide tombait en 403, son owner_getter ayant son
    propre `isdigit()`. ⚠️ Le motif est posé sur l'ENTRÉE et pas dans le handler à
    dessein : la levée se produisait dans la règle d'autz (`RESOURCE_GOVERN` →
    `ownership.can_govern` → `int(rid)`), donc AVANT que le handler soit atteint —
    une garde posée dans `_resources` n'aurait jamais été exécutée.
    """
    with pytest.raises(ValidationError):
        R.ResourceInput(op="get", resource_type=famille, resource_id=valeur)


def test_le_motif_est_publie_dans_le_contrat():
...
def test_chaque_refus_declare_est_rejoue_quelque_part():
    ...
    assert {e.code for e in cap.errors} == rejoues
