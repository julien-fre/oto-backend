...
| `oto_doc_app` | les pages/docs, **lecture seule** : sans argument = arbre de la base de connaissance de l'org active, quel que soit son nom (résolue par son ancre) ; `project_id` = arbre des pages d'un projet (enfants indentés) ; `doc_id` = une page en Markdown rendu ; `query` = recherche plein-texte avec extraits | `oto_doc` |
