...
async def _email_security(d: str) -> dict:
    """op="email_security" — posture SPF/DMARC/MTA-STS + sélecteurs DKIM courants."""
...
def register(mcp: FastMCP) -> None:
    ...
    async def infosec_domain(
        ...
        **Passif uniquement** : RDAP, DNS-over-HTTPS, logs CT publics (crt.sh),
        handshake TLS, un GET HTTP. PAS de scan de ports ni de probing de
        vulnérabilités. Aucune autorisation de la cible n'est requise : on ne
        consulte que des sources publiques / le service exposé lui-même.

        `op` :
        - **"whois"** : Domain registration via RDAP (modern structured whois).
        ...
        - **"dns"** : DNS records (A/AAAA/MX/NS/TXT) via DNS-over-HTTPS, with stack
          hints. MX and TXT records leak the company's mail provider
          (Google/Microsoft…) and SaaS verification tokens (a fingerprint of the
          tech stack).
        - **"email_security"** : E-mail authentication posture: SPF, DMARC, MTA-STS,
          common DKIM selectors. A weak/absent posture (no DMARC, or p=none) means
          the domain is easy to spoof — a maturity signal on a prospect's IT hygiene.
          ⚠️ Les sélecteurs DKIM ne sont PAS énumérables : seuls les courants sont
          testés (la réponse porte cette `note`), donc un `dkim_selectors_found`
          vide ne prouve pas l'absence de DKIM.
        - **"subdomains"** : Known subdomains via Certificate Transparency logs
          (crt.sh, passive). Reads issued-certificate names from public CT logs — no
          bruteforce, no active probing. Reveals the org's surface (api., vpn.,
          staging., mail.…).
          ⚠️ `count` = total des sous-domaines distincts trouvés, `subdomains` = la
          liste TRONQUÉE à `limit` : les deux divergent dès qu'il y en a plus. crt.sh
          renvoie souvent des 5xx transitoires (3 tentatives espacées) ; épuisées, la
          réponse porte `error` + une liste vide plutôt qu'une exception.
        - **"tls"** : TLS certificate of the host: issuer, validity, SANs, protocol
          version. SANs frequently reveal sibling domains/subdomains. A failed
          validation (expired/self-signed/hostname mismatch) is reported, not hidden
          (`validated=false` + `error` ; protocole/cipher relus sans vérification, à
          seule fin de diagnostic).
        - **"headers"** : HTTP security headers + server/tech fingerprint from a
          single GET. Checks HSTS, CSP, X-Frame-Options, X-Content-Type-Options,
          Referrer-Policy and Permissions-Policy presence, plus Server/X-Powered-By
          disclosure.

        Args:
            op: whois | dns | email_security | subdomains | tls | headers.
            domain: a domain or URL (scheme/path stripped automatically) — une
                adresse e-mail marche aussi (seule la partie après `@` est retenue).
                Pour op="subdomains", passer l'apex domain (e.g. "example.com").
        ...
        if op == "email_security":
            return await _email_security(d)
        if op == "subdomains":
