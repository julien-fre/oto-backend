"""LinkedIn — recherche société/personne B2B + enrichissement contact.

Connecteur DISTINCT d'`aiark` (#231), pas un mode/alias : même vendeur (AI Ark,
docs.ai-ark.com) et même client oto-core réutilisé tel quel
(`oto.tools.aiark.client.AiArkClient`), mais surface MCP volontairement plus
simple (pas de tool `credits`) et **app credits only** — `auth_modes=
{"platform"}` sur ce connecteur (registre `providers.py`) : pas de BYO
user/org, seul le grant plateforme posé SUR `linkedin` résout
(`resolve_api_key("linkedin")` + `record_platform_usage`). Poser la même clé
AI Ark que sous `aiark` pour partager le même pool de crédits vendeur — Oto ne
partage jamais un credential entre deux noms de connecteur (chaque connecteur
résout SON nom, ADR 0024).

v1 = endpoints SYNCHRONES seulement. Les exports/find-emails EN LOT du vendeur
...
def _verify(fields: dict, config: dict | None = None) -> None:  # noqa: ARG001 (config: contrat de sonde, non utilisé ici)
    ...
    clé invalide. Lève — le message remonte tel quel à l'UI. Utile en mode
    plateforme (op=verify auto résout le grant effectif) pour confirmer que la
    clé app credits est toujours valide.
    """
...
def register(mcp: FastMCP) -> None:
    ...
    connector_verify.register("linkedin", _verify)

    def _client() -> tuple[AiArkClient, bool]:
        key, is_platform = access.resolve_api_key("linkedin")
        return AiArkClient(api_key=key), is_platform

    def _run(fn):
        """Exécute un appel LinkedIn : traduit une erreur HTTP en McpError
        ...
        try:
            ...
            if status and status >= 500:
                msg = ("Le fournisseur LinkedIn est momentanément indisponible "
                       f"(erreur serveur {status}). Réessaie dans un moment — "
                       "ce n'est pas ton entrée.")
            elif status == 401:
                msg = ("Clé LinkedIn (app credits) invalide ou révoquée (401). "
                       "Contacte un admin plateforme.")
            else:
                msg = f"LinkedIn n'a pas pu traiter la requête ({e})."
            raise McpError(ErrorData(code=INVALID_PARAMS, message=msg))
        if is_platform:
            access.record_platform_usage("linkedin")
        return result
    ...
    def linkedin_company_search(
        ...
        """Search B2B companies by firmographics (LinkedIn data).

        Args:
            account: filter object. Nested DSL — each field takes an
        ...
        Returns the raw page: `content[]` (company records with summary, link,
        contact, financial, location, technologies…), `totalElements`,
    ...
    def linkedin_people_search(
        ...
        """Search B2B people by company + contact filters (LinkedIn data).

        Args:
            account: filters on the person's company (same DSL as
                linkedin_company_search), e.g. {"domain": {"any": {"include": ["amazon.com"]}}}.
        ...
        Returns the raw page: `content[]` (people with profile, link, location,
        company, department, skills…), `totalElements`, `totalPages`,
        `trackId`. Note: search results do NOT include emails — use
        linkedin_export_person to get an email for a specific match.
        """
    ...
    @mcp.tool()
    def linkedin_export_person(
        id: Optional[str] = None,
        url: Optional[str] = None,
    ) -> dict:
        """Export one person WITH email (synchronous email finder, LinkedIn data).

        Args:
            id: a person id from a prior linkedin_people_search result, OR…
        ...
        if not id and not url:
            raise McpError(ErrorData(
                code=INVALID_PARAMS,
                message="linkedin_export_person exige `id` ou `url`.",
            ))
    ...
    @mcp.tool()
    def linkedin_reverse_lookup(search: str) -> dict:
        """Reverse-lookup a person from a contact detail (LinkedIn data).
    ...
    @mcp.tool()
    def linkedin_mobile_phone(
        linkedin: Optional[str] = None,
        domain: Optional[str] = None,
        name: Optional[str] = None,
    ) -> dict:
        """Find a person's mobile phone number(s) (LinkedIn data).
        ...
        if not linkedin and not (domain and name):
            raise McpError(ErrorData(
                code=INVALID_PARAMS,
                message="linkedin_mobile_phone exige `linkedin` OU (`domain` ET `name`).",
            ))
