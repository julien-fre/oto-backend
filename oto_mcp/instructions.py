...
**Porte ton contexte DANS l'appel, jamais dans un état de session.** Il n'y a AUCUN état de session serveur (ADR 0038) : quand une action dépend d'un contexte précis, passe-le EN PARAMÈTRE de l'appel. **Les jetons de contexte sont préfixés `_`** (ils sont à la plateforme ; sans préfixe, le nom appartient aux arguments métier de l'outil) — `_project=<id>` (le jeton PRIMAIRE : org du projet, slots `slot:<nom>`, identités connecteur préfaites), `_org=<id>` / `_group=<id>` (agir dans une org/équipe donnée), `_account=<id>` (le compte/identité à OPÉRER quand plusieurs sont possibles : un credential parmi plusieurs « 2 Zoho », OU le compte LinkedIn/messagerie sous la clé partagée — **y compris un compte qu'un pair t'a accordé** ; `oto_identity(op='list')` les liste, pin ÉPHÉMÈRE cet appel), `_instance=<ref>` (une instance de connecteur PRÉCISE, refs via `oto_instance(op='list')`), `_run_id=<id>` (rattacher l'appel à un `run_start`). Les `oto_use_*` ne posent plus d'état : ils valident l'accès et te rappellent le jeton à passer. **Un appel refusé « plusieurs comptes … aucun par défaut » n'est PAS une erreur à contourner en devinant** : liste-les (`oto_identity(op='list')`) et DEMANDE à l'utilisateur lequel utiliser pour cet appel (`_account=`), plutôt que d'en choisir un au hasard — un identifiant de credential mal choisi route silencieusement l'action sous le mauvais compte externe. Si l'utilisateur veut que ce choix devienne permanent, pose-le avec `oto_identity(op='set')`.
