...
CAPABILITIES += [
    Capability(
        ...
        description=(
            ...
            "created or renamed. ⚠️ That scope is the WHOLE reach, so the graph is not "
            "symmetric: a page in the org KB resolves against the KB alone, so it can "
            "NEVER link to a page living in a project — while that project page links "
            "back to it fine. A page can therefore be cited from the org's top map and "
            "still read as an orphan here: do not use backlinks as a completeness or "
            "orphan check without knowing that. Every write says which of its `[[…]]` "
