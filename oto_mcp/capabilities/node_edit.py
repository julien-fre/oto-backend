...
from ..db import nodes as db_nodes, node_view as db_node
...
def _create(ctx: ResolvedCtx, inp: NodeEditInput) -> dict:
    ...
    if not (inp.title or "").strip():
        raise AuthzDenied(400, "missing_title", "`title` requis pour créer une page.")
...
def _update(ctx: ResolvedCtx, inp: NodeEditInput) -> dict:
    ...
    fait = db_nodes.update_page(fiche["id"], title=inp.title,
                                description=inp.description, body_md=inp.body_md)
    if not fait:
        raise AuthzDenied(400, "rien_a_ecrire",
                          "Aucun champ fourni — `title`, `description` ou `body_md`.")
...
CAPABILITIES += [
    Capability(
        ...
        description=(
            ...
            "delete). Nodes written here are NATIVE: they have no source in the old "
            "world, nothing refreshes them, and the old surfaces (`oto_doc`, "
            "`oto_project`) do not see them — the two universes live side by side "
            "during the transition. A page BODY is stored as ordered blocks with "
            "STABLE ids, so editing a title never re-identifies the paragraphs and "
            "citations survive. `scope` picks the owner (platform | org | group | "
            "user, default org) and follows the SAME write ladder as guides. `move` "
