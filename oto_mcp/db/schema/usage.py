...
-- COMPTE et pour TOUTE désinscription, comme `outreach_optouts`
-- (`db/schema/outreach.py`) et pour la même raison : ce refus vise un CANAL, pas un
-- signal (donc pas une colonne sur `usage_signals`, qui survivrait mal à un
-- ré-arbitrage) ni une identité (donc pas une colonne sur `users` : elle porte sa
-- propre date et sa provenance, un DELETE la lève sans toucher au compte).
