...
- `docs/conventions.md` — **les règles de travail du backend**, chacune née d'un incident daté (test qui décrit le système et non l'intention, garde-fou exercé sur le montage réel, aucune adresse en dur, contrainte mono-loop, cycle d'un connecteur). À lire avant d'écrire du code ici.
...
- `docs/event-loop-perf.md` — les **3** modes de gel mono-loop + protections + recettes py-spy/aiodebug. ⚠️ Le 3ᵉ (27/08) n'est pas un I/O mal placé mais une **requête lente** (JSONB sans index d'expression : 185 s de boucle tenue, prod + tenant tiers à terre) — même signature py-spy que le 2ᵉ, remède opposé : indexer, pas déplacer. Corollaire de méthode : une lecture dont le coût suit le VOLUME TOTAL et non le scope demandé est une panne à retardement, qui se déclenche sans déploiement.
